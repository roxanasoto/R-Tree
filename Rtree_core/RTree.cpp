...
/*
	Obtener las regiones del árbol
*/

void RTree::getRegion(Node *node, vector<Region> &list)
{
	if (node == NULL)
		return;

	if (node->region != NULL)
		list.push_back(*node->region);
	for (int i = 0; i < node->children.size(); i++)
	{
		getRegion(node->children.at(i), list);
	}
}
/*
