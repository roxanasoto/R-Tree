...
bool RTree::insertElement( Polygon pol)
{
    ...
    else //Si no es hoja , root es nodo interno
    {
       ...
       Node* node = searchRegion(root, obj.getMbr()); //Retorna un nodo interno h -1 //nodo interno donde puede ser insertado
       if(node != NULL)
       {
           //Buscar en que region de node->keyregion debs ser insertado
           Region minMbr = calcularMbr(node->keyRegion.at(0), obj.getMbr());
           int idxTemp = 0;
           for(int i = 1 ; i <  node->keyRegion.size(); i++)
           {
               Region temp = calcularMbr(node->keyRegion.at(i), obj.getMbr());
